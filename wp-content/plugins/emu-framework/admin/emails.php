...
switch( post_val( 'reset' ) )
{
	...
	case 'Reset All':
		...
		$emuApp->getManager( 'email-template' )->populateTemplates(); break;

	case 'Refresh':

		// Reload any new templates
		$emuApp->getManager('email-template')->refreshTemplates(); break;

}

$templates = $emuApp->getManager( 'email-template' )->getTemplates();
...
jQuery(document).ready(function() {
	...
	jQuery('input[value="Reset All"]').click(function() {
		...
		if( confirm( 'Are you sure you want to reset all emails?' ) )
		{
			if( confirm ( 'Really? All email changes will be lost!' ) )
			{
				if( confirm( 'Really?' ) )
				{
					return true;
...
<form method="post" action="" />
	...
	<div class="wrap emu-templates">
		...
		<h2><?php echo $emuApp->menuName?> Email Templates</h2>
		...
		<input type="hidden" name="templateType" id="templateType" value="email" />
