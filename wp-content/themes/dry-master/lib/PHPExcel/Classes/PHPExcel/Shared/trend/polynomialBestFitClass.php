...
class PHPExcel_Polynomial_Best_Fit extends PHPExcel_Best_Fit
{
	...
	private function _polynomial_regression($order, $yValues, $xValues, $const) {
		...
		$meanX = array_sum($xValues) / count($xValues);
		$meanY = array_sum($yValues) / count($yValues);
		$xx_sum = $yy_sum = $xy_sum = 0;
		...
		$matrixA = new PHPExcel_Shared_JAMA_Matrix($A);
		$matrixB = new PHPExcel_Shared_JAMA_Matrix($B);
		...
		$this->_calculateGoodnessOfFit($x_sum,$y_sum,$xx_sum,$yy_sum,$xy_sum, $meanX, $meanY);
